...
class PurchaseBillRepository implements IPurchaseBillRepository {
  ...
  @override
  Future<Result<PurchaseBillDetails>> getBillDetails(int billId) => remote
      .getBillDetails(billId)
      .handleRepository(apiBillDetailsMapper);

  @override
  Stream<Result<List<PurchaseBill>>> getBills(
      int pageNumber, int pageSize, int? state) => remote.getBills(pageNumber, pageSize, state).handleRepository((p0) => null)

  @override
  Future<Result<bool>> updateBills(int billId, PurchaseBillRequest request) {
    // TODO: implement updateBills
  ...
  PurchaseBillDetails apiBillDetailsMapper(ApiPurchaseBillDetails api) => api.toDomain();
}
